...
void agregarMovimiento(vector<string> comandos) {
    ...
    int r = ::agregarMovimiento(estado.colaComandos,comandos[1],stod(comandos[2]),comandos[3]);
    if(r==-1){
        cout<<"(Error) No se pudo agregar el movimeinto"<<endl;
    }
    else{
...
void agregarAnalisis(vector<string> comandos, const string& lineaOriginal) {
    ...
    if (lineaOriginal.find('\'')!=string::npos) {
        comentario = extraerComentario(lineaOriginal);
        if (comentario.empty()) {
            cout << "(Formato erroneo) El comentario debe estar entre comillas simples." << endl;
            return;
        }
    }
    int r= ::agregarAnalisis(estado.colaComandos,comandos[1],comandos[2],comentario);
    if(r==-1){
        cout<<"(Error) No se pudo agregar el analisis." << endl;
    }
    else{
...
void agregarElemento(vector<string> comandos) {
    ...
    int r = ::agregarElemento(estado.listaElementos,comandos[1],stod(comandos[2]),comandos[3],stod(comandos[4]),stod(comandos[5]));
    if(r==-1){
        cout<<"(Error) No se pudo agregar el elemento"<<endl;
    }
    else{
